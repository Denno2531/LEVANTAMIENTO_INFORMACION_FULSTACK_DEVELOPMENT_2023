    ...
    if ($result = $conexion->query($sql)) {
        if ($row = mysqli_fetch_array($result)) {
            ...
            if (!empty($_POST['remember_session'])) {
                ...
                setcookie('pass', $_POST['txtpass'], time() + 15 * 24 * 60 * 60);
            ...
            echo '
                    ...
                    <input type="text" class="text" name="txtuser" placeholder="Usuario o Correo" value="' . ($_COOKIE['user'] ?? '') . '" autofocus required />
                    <input type="password" class="textcontrasena" name="txtpass" placeholder="Contraseña" value="' . ($_COOKIE['pass'] ?? '') . '" maxlength="8" autocomplete="off" required />
                    <div class="forgot-pass">
                       <a class="un" href="#" onclick="mostrarRecuperacion()">¿Olvidaste la contraseña?</a>
                    </div>
                    <div class="pretty p-svg p-curve p-smooth">
                        <input type="checkbox" name="remember_session" placeholder="Recordar" value="1" ' . ($_COOKIE['remember'] ?? '' ? 'checked' : '') . ' />
    ...
    echo '
            ...
            <div class="pretty p-svg p-curve p-smooth">
                ...
                <div class="state p-primary">
                    ...
                    <label class="remember">Recuerdame</label>
                </div>
            </div>
            <button class="button" type="submit">Entrar</button>
        ';
