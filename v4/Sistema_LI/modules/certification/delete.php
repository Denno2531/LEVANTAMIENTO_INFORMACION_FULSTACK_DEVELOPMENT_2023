...
if(empty($nombreArchivoEvidencia)){
    $sql= "SELECT evidencepdf FROM certification WHERE archivopdf = '" . $nombreArchivo . "'";
...
if (!empty($_POST['txtuserid'])) {
    ...
    $rutaArchivo = 'certificadopdf/' . $_SESSION["user"] . '/' . $nombreArchivo;
    ...
    if (!empty($nombreArchivoEvidencia)) {
        ...
        $rutaArchivoEvidencia = '../edit_send_one/certificadopdf/' . $_SESSION["user"] . '/' . $nombreArchivoEvidencia;
