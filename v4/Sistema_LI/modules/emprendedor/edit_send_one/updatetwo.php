...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($_FILES["archivo"]["error"] === UPLOAD_ERR_OK) {
		...
		if (move_uploaded_file($ruta_temporal, $ruta_destino)) {
			...
			$sql_update = "UPDATE send_two SET estado = '" . trim($_POST['txtestado']) . "', message = '" . trim($_POST['descripcion']) . "', evidencepdf = '" . $nombre_archivo . "',  updated_at = '". $date ."' WHERE num = '" . trim($_POST['txtnum']) . "'";
		...
		$sql_update = "UPDATE send_two SET estado = '" . trim($_POST['txtestado']) . "', message = '" . trim($_POST['descripcion']) . "',  updated_at = '". $date ."' WHERE num = '" . trim($_POST['txtnum']) . "'";
